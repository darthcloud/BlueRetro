...
int32_t wireless_to_generic(struct bt_data *bt_data, struct generic_ctrl *ctrl_data) {
    ...
    if (to_generic_func[bt_data->dev_type]) {
        ret = to_generic_func[bt_data->dev_type](bt_data, ctrl_data);
        }
...
void wireless_fb_from_generic(struct generic_fb *fb_data, struct bt_data *bt_data) {
    if (fb_from_generic_func[bt_data->dev_type]) {
        fb_from_generic_func[bt_data->dev_type](fb_data, bt_data);
        }
}
