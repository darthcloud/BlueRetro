...
static struct macro macros[] = {
    {.macro = SYS_RESET, .action = sys_mgr_reset, .flag_mask = WIRED_WAITING_FOR_RELEASE_MACRO1},
...
static void check_macro(int32_t value, uint32_t map_mask, struct macro *macro, atomic_t *flags) {
    if ((map_mask & macro->macro) == macro->macro) {
...
void sys_macro_hdl(int32_t dev_mode, struct generic_ctrl *ctrl_data, struct wired_data *wired_data) {
    if (dev_mode == DEV_PAD || dev_mode == DEV_PAD_ALT) {
        ...
        for (uint32_t i = 0; i < sizeof(macros)/sizeof(macros[0]); i++) {
            ...
            check_macro(value, map_mask, macro, &wired_data->flags);
        }
    }
}
