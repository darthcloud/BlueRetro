...
static void bt_sdp_cmd_svc_search_attr_rsp(uint16_t handle, uint16_t cid, uint8_t tid) {
...
void bt_sdp_hdlr(struct bt_dev *device, struct bt_hci_pkt *bt_hci_acl_pkt) {
    switch (bt_hci_acl_pkt->sdp_hdr.op_code) {
        ...
        {
            bt_sdp_cmd_svc_search_attr_rsp(device->acl_handle, device->sdp_chan.dcid, bt_hci_acl_pkt->sdp_hdr.tid);
            break;
        }
    }
}
