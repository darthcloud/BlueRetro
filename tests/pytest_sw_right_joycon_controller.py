...
from device_data.sw import sw_n_jc_btns_mask, sw_n_rjc_axes
...
def test_sw_right_joycon_controller_default_buttons_mapping(blueretro):
    ...
    blueretro.get_logs()
    blueretro.expect(r'# dev: 0 type: 5:12 Joy-Con \(R\)', timeout=1)
    ...
    for sw_btns, br_btns in btns_generic_test_data(sw_n_jc_btns_mask):
        if br_btns in (65536, 131072, 262144, 524288, 1048576, 33554432, 536870912, 993984512):
...
def test_sw_right_joycon_controller_axes_default_scaling(blueretro):
...
def test_sw_right_joycon_controller_axes_scaling_with_calib(blueretro):
    ''' Set the various axes and check if the scaling is right. '''
    # Connect device
    blueretro.disconnect()
    blueretro.send_system_id(GC)
