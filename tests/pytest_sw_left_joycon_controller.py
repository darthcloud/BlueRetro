...
from device_data.sw import sw_n_jc_btns_mask, sw_n_ljc_axes
...
def test_sw_left_joycon_controller_default_buttons_mapping(blueretro):
    ...
    blueretro.get_logs()
    blueretro.expect(r'# dev: 0 type: 5:11 Joy-Con \(L\)', timeout=1)
...
def test_sw_left_joycon_controller_axes_default_scaling(blueretro):
...
def test_sw_left_joycon_controller_axes_scaling_with_calib(blueretro):
    ''' Set the various axes and check if the scaling is right. '''
    # Connect device
    blueretro.disconnect()
    blueretro.send_system_id(GC)
