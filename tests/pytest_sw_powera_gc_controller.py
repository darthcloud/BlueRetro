...
from device_data.sw import sw_n_pwa_gc_btns_mask, sw_n_axes
...
def test_sw_powera_gc_default_buttons_mapping(blueretro):
...
def test_sw_powera_gc_controller_axes_default_scaling(blueretro):
...
def test_sw_powera_gc_axes_scaling_with_calib(blueretro):
    ''' Set the various axes and check if the scaling is right. '''
    # Connect device
    blueretro.disconnect()
    blueretro.send_system_id(GC)
